PG_MAJOR ?= 17
DEV_IMAGE  = pg_seaturtle-dev:pg$(PG_MAJOR)
IMAGE      = pg_seaturtle:pg$(PG_MAJOR)
TARGET_VOL      = pg_seaturtle_target_pg$(PG_MAJOR)
CARGO_VOL       = pg_seaturtle_cargo
QUERY_CONTAINER = pg_seaturtle_query
...
cargo: dev-image
	docker run --rm -v $(CURDIR):/build/pg_seaturtle -v $(TARGET_VOL):/build/pg_seaturtle/target \
...
run: image
	docker run --rm --name pg_seaturtle -p 5432:5432 -e POSTGRES_PASSWORD=postgres $(IMAGE) \
		-c shared_preload_libraries=pg_seaturtle \
		-c log_min_messages=log

psql:
	docker exec -it pg_seaturtle psql -U postgres
...
run-sql: image
	...
	@docker logs $(QUERY_CONTAINER) 2>&1 | grep -E 'pg_seaturtle|LOG:' || true
...
# Show pg_seaturtle log lines from the running container
logs:
	@docker logs pg_seaturtle 2>&1 | grep -E 'pg_seaturtle' || true

logs-all:
	@docker logs pg_seaturtle 2>&1

logs-follow:
	@docker logs -f pg_seaturtle 2>&1
...
integration-test: $(VENV)/.stamp
	@for v in $(PG_VERSIONS); do \
		...
		PG_SEATURTLE_IMAGE=pg_seaturtle:pg$$v $(VENV)/bin/pytest tests/ -v; \
	done
...
clean:
	docker volume rm pg_seaturtle_target_pg17 pg_seaturtle_target_pg18 $(CARGO_VOL) 2>/dev/null || true
