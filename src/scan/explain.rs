...
use super::exec::segments::ScanBufferStats;
use super::exec::{AggScanState, CountScanState, DecompressState, MinMaxScanState};
...
pub unsafe extern "C-unwind" fn explain_custom_scan(
    ...
    unsafe {
        pg_sys::ExplainPropertyText(
            c"Storage".as_ptr(),
            c"Compressed (DeltaXDecompress)".as_ptr(),
            es,
        );
...
pub unsafe extern "C-unwind" fn explain_deltax_append(
    ...
    unsafe {
        pg_sys::ExplainPropertyText(
            c"Storage".as_ptr(),
            c"Compressed (DeltaXAppend)".as_ptr(),
            es,
        );
        explain_timing(node, es);
    }
...
pub unsafe extern "C-unwind" fn explain_count_scan(
    ...
    unsafe {
        pg_sys::ExplainPropertyText(
            c"Storage".as_ptr(),
            c"Compressed (DeltaXCount - COUNT(*) pushdown)".as_ptr(),
            es,
        );
        ...
        let total_ms = (state.metadata_us + state.heap_scan_us) as f64 / 1000.0;
        emit_text(
            es,
            c"DeltaX Timing",
            &format!(
        ...
        emit_text(
            ...
            &format!(
                ...
                state.total_count, state.total_segments,
            ),
        );

        if (*es).buffers {
            emit_buf_stats(es, &state.buf_stats);
        }
    }
}
...
pub unsafe extern "C-unwind" fn explain_minmax_scan(
    ...
    unsafe {
        pg_sys::ExplainPropertyText(
            c"Storage".as_ptr(),
            c"Compressed (DeltaXMinMax - metadata-only aggregate)".as_ptr(),
            es,
        );
        ...
        emit_text(
            es,
            c"DeltaX Stats",
            &format!("{} segments={}", agg_parts.join(", "), state.total_segments),
        );
...
pub unsafe extern "C-unwind" fn explain_agg_scan(
    ...
    unsafe {
        pg_sys::ExplainPropertyText(c"Storage".as_ptr(), c"Compressed (DeltaXAgg)".as_ptr(), es);
        ...
        // Wall time supersedes the sum-of-phases total when it was captured;
        // the phase counters double-count on parallel paths.
        ...
        let regex_str = if state.regex_cache_size > 0 {
            format!(
                " regex_cache_size={} regex_calls={}",
                state.regex_cache_size, state.regex_cache_calls
            )
        } else {
            String::new()
        };
        let filtered_str =
            if state.segments_metadata_resolved > 0 || state.segments_decompressed > 0 {
                format!(
                    " segments_metadata_resolved={} segments_decompressed={}",
        ...
        if state.topn_limit > 0 {
            ...
            emit_text(
                ...
                &format!(
                    ...
                    state.topn_limit, state.topn_sort_col, direction, state.pre_topn_groups,
                ),
            );
        }
        ...
        if state.blob_cache_hits + state.blob_cache_misses > 0 {
            emit_text(
                ...
                &format!(
                    ...
                    state.blob_cache_hits, state.blob_cache_misses, state.blob_cache_bytes_served,
                ),
            );
        }
...
/// Shared timing/stats output for EXPLAIN ANALYZE on the decompression scan.
unsafe fn explain_timing(node: *mut pg_sys::CustomScanState, es: *mut pg_sys::ExplainState) {
    unsafe {
        if !(*es).analyze {
            return;
        }
        ...
        // Aggregate leader + all worker slots for the top-line totals so
        // EXPLAIN shows total work across the parallel group rather than
        // just the leader's slice. Slot 0 is the leader; its counters are
        // already in `t`, so skip it during aggregation to avoid
        // double-counting.
        ...
        let total_ms =
            (metadata_us + heap_scan_us + decompress_us + batch_eval_us + emit_us) as f64 / 1000.0;
        emit_text(
            es,
            c"DeltaX Timing",
            &format!(
        ...
        let topn_str = if t.topn_limit > 0 {
            format!(
                " topn={} topn_candidates={} topn_p2_segs={}",
                t.topn_limit, t.topn_candidates, t.topn_phase2_segments
            )
        } else {
            String::new()
        };
        ...
        // EXPLAIN VERBOSE → surface per-process segment counts + decompress
        // timing so reviewers can spot worker-balance issues at a glance.
        if !state.cached_worker_timings.is_empty() && (*es).verbose {
        ...
        // Per-phase shared-buffer deltas. Custom-scan work happens in
        // BeginCustomScan, outside PG's node-level instrumentation, so the
        // standard `Buffers:` line misses it. This surfaces the breakdown for
        // cold-cache investigations.
        if (*es).buffers {
            emit_buf_stats(es, &t.buf_stats);
        }
    }
}

/// Convenience wrapper around `ExplainPropertyText` for runtime-formatted
/// values. The label is always a static `c""` literal so we avoid the
/// allocation there; only the value needs CString conversion.
unsafe fn emit_text(es: *mut pg_sys::ExplainState, label: &CStr, value: &str) {
    let c = std::ffi::CString::new(value).expect("EXPLAIN property contained NUL");
    unsafe {
        pg_sys::ExplainPropertyText(label.as_ptr(), c.as_ptr(), es);
    }
}

/// Single source for the "DeltaX Buffers" line — every scan node emits the
/// same six-field shape, so keep the format string in one place.
unsafe fn emit_buf_stats(es: *mut pg_sys::ExplainState, b: &ScanBufferStats) {
    unsafe {
        emit_text(
            es,
            c"DeltaX Buffers",
            &format!(
                ...
                b.meta_hit, b.meta_read, b.bloom_hit, b.bloom_read, b.blob_hit, b.blob_read,
            ),
        );
    }
}
