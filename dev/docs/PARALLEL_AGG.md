...
### Phase D — Parallel COUNT(DISTINCT) for dict-encoded text ✅ INFRASTRUCTURE DONE (doesn't help JSONBench Q1)
...
- `agg.rs::build_dict_distinct_remaps` — **parallel** leader pre-pass via two `std::thread::scope` calls. Phase 1 (parallel) builds per-thread local interners + per-seg local remaps. Phase 2 (sequential) merges local interners into a global one and produces a `thread_to_global[t][local_thread_id] = global_id` translation table. Phase 3 (parallel) rewrites per-seg remaps to global IDs. Two-stage eligibility: `dict_size_sum > PHASE_D_MAX_DICT_SIZE_SUM (50M)` peeks the 4-byte dict-size header from each blob without LZ4-decompressing — bails fast on UUID-style cols; `global_count > PHASE_D_MAX_GLOBAL_FOR_BITSET (10M)` is checked AFTER the pre-pass to bound bitset memory.
...
**JSONBench impact: zero**. Q1 was the design target, but `x_did` is **Lz4Blocked-encoded**, not dict-encoded — confirmed via the eligibility check on EC2 (comp_byte=6 = Lz4Blocked at segment 0). Per-segment cardinality of `did` is ~25K-30K unique out of 30K rows, which fails the compressor's `cardinality <= rows/2` heuristic at ingest time. Phase D's bitset path requires every relevant segment to be dict-encoded for the column; Q1's eligibility check correctly bails on the first non-dict segment and the path falls back to the existing `HashSet<u128>` route at 2.1s warm.

**Where Phase D would actually fire**: any GROUP BY + COUNT(DISTINCT) shape where the DISTINCT column is dict-encoded. `should_use_dictionary` requires per-seg cardinality < 65536 AND ≤ rows/2, so columns with high redundancy (status flags, terminal IDs, product categories, low-cardinality enums). Tested via `test_phase_d_count_distinct_low_card_text`.

**What Q1 actually needs**: the `did` column's count-distinct cost is dominated by Lz4Blocked decompression + per-row hashing, neither of which Phase D addresses. Options for a follow-up that *would* help Q1:
- **Pre-hash dict-equivalent at compress time**: store a side-table of `Vec<u128>` hashes per Lz4Blocked text column, populated at compression. Per-row work becomes `seg.text_hashes[row]` instead of `hash128_str(get_str(row).as_bytes())`. Saves the hashing step but not the decompression.
- **Different storage encoding for high-cardinality count-distinct columns**: e.g. compress `did` as a stable hash representation (xxhash3 / fingerprint) rather than the raw string. Per-row work becomes integer compare. Bigger architectural shift; needs a column-level opt-in via `deltax_enable_compression`.
- **Approximate count-distinct** (HLL): O(1)-state per group; matches ClickHouse's default behaviour but loses exactness.

None of these are "Phase D" any more — call them Phase G if/when prioritised.
