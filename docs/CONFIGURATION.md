...
| `pg_deltax.condition_cache` | `on` | userset | Cache each segment's qual-selection bitmap (which rows pass a given set of `WHERE` filters) in the blob cache's shared memory, keyed by `(segment, filter fingerprint)`. Later queries with identical filters skip predicate evaluation and the decompression of filter-only columns — repeated filtered queries (e.g. text `LIKE`) run 2–4× faster warm. Inert when the blob cache is unavailable (`blob_cache_mb = 0` or session preload mode). Userset so it can be toggled per session for A/B runs. See `dev/docs/CONDITION_CACHE.md`. |
